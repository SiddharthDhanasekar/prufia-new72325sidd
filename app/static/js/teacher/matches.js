$(document).ready(function() {
    ...
    function getConfidenceDescription(score) {
        if (score >= 85) return 'Strong Match (Authored by member)';
        if (score >= 70) return 'Likely Match (May include AI tools or human help)';
        return 'Probable Mismatch (Unlikely to be member\'s work)';
    }
    ...
    function filterResults() {
        const memberFilter = $('#memberFilter').val();
    ...
    $('#memberFilter, #fileFilter, #confidenceFilter').on('change', filterResults);
    $('.filter-reset').on('click', function() {
        $('#memberFilter, #fileFilter, #confidenceFilter').val('');
        filterResults();
