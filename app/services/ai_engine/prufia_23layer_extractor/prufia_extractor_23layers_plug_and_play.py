...
def extract_23_layer_profile(text):
    words = text.split()
    word_count = len(words)
    unique_words = set(words)
    def safe_div(n, d):
        return round(n / d, 3) if d != 0 else 0.0
    def count_matches(keys):
        return sum(1 for w in words if w.lower() in keys)
    def count_phrase_matches(text, phrase_list, threshold=0.48):
        text = text.lower()
        ...
        for phrase in phrase_list:
            ...
            for sentence in sentences:
                similarity = SequenceMatcher(None, sentence, phrase).ratio()
                if similarity >= threshold:
                    match_count += 1
                    break
        return match_count

    BIOGRAPHICAL_CONSISTENCY_PHRASES = [
    ...
    METACOGNITIVE_AWARENESS_PHRASES = [
    ...
    ATTENTION_FRAGMENTATION_PHRASES = [
    ...
    trait_profile = {}
    # Stylometric (7)
    trait_profile['sf'] = round(len(set(words)) / word_count * 100, 2)
    trait_profile['pf'] = round(text.count(",") + text.count(".") + text.count(";"), 2)
    trait_profile['eb'] = round(len(set(words)) / len(words), 2) if words else 0.0
    trait_profile['sm'] = round(sum(1 for w in words if len(w) > 6), 2)
    trait_profile['tt'] = round(sum(len(w) for w in words) / len(words), 2) if words else 0.0
    trait_profile['mc'] = len(set(words))
    trait_profile['pgfi'] = round(sum(ord(c) for c in text) % 100 / 10.0, 2)
    # Simple behavioral traits (keep as-is)
    trait_profile['vocab_diversity'] = safe_div(len(unique_words), word_count)
    trait_profile['sent_len_dist'] = safe_div(sum(len(w) for w in words), word_count)
    trait_profile['info_density'] = safe_div(len([w for w in words if len(w) > 8]), word_count)
    trait_profile['academic_vocab_density'] = safe_div(count_matches(["paradigm", "theory", "analysis"]), word_count)
    trait_profile['knowledge_integration'] = count_matches(["because", "therefore", "which suggests"])
    trait_profile['discourse_flow'] = count_matches(["first", "next", "then", "finally"])
    # TRUST BOOSTER TRAITS – now using fuzzy matching
    trait_profile['emotion_gradient'] = count_phrase_matches(text, EMOTION_GRADIENT_PHRASES)
    trait_profile['revision_artifacts'] = count_phrase_matches(text, REVISION_ARTIFACTS_PHRASES)
    trait_profile['context_adaptation'] = count_phrase_matches(text, CONTEXT_ADAPTATION_PHRASES)
    trait_profile['metacognitive_awareness'] = count_phrase_matches(text, METACOGNITIVE_AWARENESS_PHRASES)
    trait_profile['biographical_consistency'] = count_phrase_matches(text, BIOGRAPHICAL_CONSISTENCY_PHRASES)
    trait_profile['attention_fragmentation'] = count_phrase_matches(text, ATTENTION_FRAGMENTATION_PHRASES)
    trait_profile['cultural_knowledge'] = count_phrase_matches(text, CULTURAL_KNOWLEDGE_PHRASES)
    trait_profile['domain_expertise'] = count_phrase_matches(text, DOMAIN_EXPERTISE_PHRASES)
    trait_profile['creative_synthesis'] = count_phrase_matches(text, CREATIVE_SYNTHESIS_PHRASES)
    trait_profile['temporal_awareness'] = count_phrase_matches(text, TEMPORAL_AWARENESS_PHRASES)
